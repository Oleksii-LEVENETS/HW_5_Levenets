...
def test_create_member(create_user):
...
def test_access_member(create_user):
    member_1 = create_user
    assert set_like_to_article(member_1, 1) is True
    assert share_article(member_1, 1) is True
...
def test_access_moderator():
    moderator_1 = Moderator(badge="New_Badge_Moderator", **change)
    assert set_like_to_article(moderator_1, 1) is True
    assert share_article(moderator_1, 1) is True
    assert create_article(moderator_1, 1) is True
    assert update_article(moderator_1, 1) is True
    with pytest.raises(NoAccess):
...
def test_access_admin():
    admin_1 = Admin(badge="Badge_Admin", **params)
    assert set_like_to_article(admin_1, 1) is True
    assert share_article(admin_1, 1) is True
    assert create_article(admin_1, 1) is True
    assert update_article(admin_1, 1) is True
    assert delete_article(admin_1, 1) is True
    ...
    with pytest.raises(NoAccess):  # admin_1.level = 2
        delete_group(admin_1, 1)
    admin_1.level = 3
    assert delete_group(admin_1, 1) is True  # admin_1.level = 3
    admin_1.level = 4
    assert delete_group(admin_1, 1) is True  # admin_1.level = 4
